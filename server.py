...
async def _call(request: Request, fn):
    ...
    try:
        ...
        return _json({"error": "색인이 없습니다", "need_index": True, "index": idx.status()})
    except Exception as e:  # noqa: BLE001 — 색인이 깨졌거나 못 읽는 경우
        traceback.print_exc()
        return _json({"error": f"색인을 열지 못했습니다: {type(e).__name__}: {e}"})
