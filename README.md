...
「지금 씬에 넣기」가 굴린 태그를 캐릭터 카드와 스타일 카드에 넣습니다.
...
| 자리 | `<앱 뿌리>\data\plugins\tag-roll\` |
| 지우기 | 창 아래 「색인 관리」 |

★색인을 **플러그인 폴더 안에 두지 않습니다.** 앱은 플러그인을 업데이트할 때 옛 폴더를
`_old-tag-roll-<시각>` 으로 물리고 새 것을 놓으므로, 안에 두면 판을 올릴 때마다 960MB 를 다시 받게 됩니다.
