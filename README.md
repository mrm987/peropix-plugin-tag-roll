...
`굴리기_신체`·`굴리기_복장`·`굴리기_표정`·`굴리기_포즈`·`굴리기_NSFW`·`굴리기_씬` 여섯입니다.
인원이 둘 이상이면 인물마다 앞의 넷이 각자의 캐릭터 카드에 들어갑니다 (씬·NSFW 는 한 벌). 이름표도 언어를 따라가서
영어에서는 `Roll_Design`, 일본어에서는 `ロール_デザイン` 꼴이 됩니다.

넣기를 누르면 **이 접두어가 붙은 블록을 먼저 전부 지우고** 이번에 굴린 것만 넣으므로, 이번에 안 뽑힌 모듈의
지난 태그가 남아 섞이지 않습니다. 언어를 바꾸어 다시 넣어도 옛 이름의 블록을 함께 걷어 냅니다. 접두어가 없는
블록(직접 만드신 것)은 건드리지 않으니, 굴린 결과를 남겨 두고 싶으시면 블록 이름을 바꿔 두시면 됩니다.
...
| 지우기 | 플러그인을 지우면 색인도 함께 휴지통으로 갑니다 (색인만 따로 지우는 길은 두지 않았습니다) |
...
- `web/i18n.js`·`msgs.py` — 화면과 창구의 문구 (한국어 원문이 키이고, 영어·일본어만 표로 둡니다).
